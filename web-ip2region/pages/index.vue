...
const cookieAccepted = ref(import.meta.client ? !!localStorage.getItem('ip2region-cookie-ok') : true)

const acceptCookies = () => {
  cookieAccepted.value = true
  localStorage.setItem('ip2region-cookie-ok', '1')
}
...
const getInitialTheme = (): 'dark' | 'light' => {
  if (import.meta.client) {
    const saved = localStorage.getItem('ip2region-theme')
...
const toggleTheme = () => {
  ...
  localStorage.setItem('ip2region-theme', mapTheme.value)
  if (map.value) {
      ...
      <section class="cnp-nav-wrap">
        <header class="cnp-nav">
          <div class="cnp-nav-bar">
            <a class="cnp-nav-brand" href="/" aria-label="ip2region.io 首页" @click.prevent="hardReloadHome">
              <img class="cnp-nav-logo-image" :src="logoUrl" alt="ip2region.io">
            </a>
          ...
          <transition name="cnp-dropdown">
            <div v-show="menuOpen" class="cnp-nav-panel" @mouseenter="menuOpen = true" @mouseleave="menuOpen = false">
              ...
              <div class="cnp-panel-text"><strong>本地离线数据库</strong><span>仅使用 ip2region IPv4 / IPv6 数据</span></div>
              </div>
              <div class="cnp-panel-card">
      ...
      <footer v-if="data" id="footer" class="cnp-footer">
        ...
        <div class="cnp-footer-pill-wrap">
          <span class="cnp-footer-pill">
            <span class="cnp-pill-item">© {{ currentYear }} ip2region.io</span>
